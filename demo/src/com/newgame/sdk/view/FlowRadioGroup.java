...
/** 鍙互鏀惧绉嶅竷灞�帶浠讹紝鑳芥壘鍒皉adiobutton */
public class FlowRadioGroup extends LinearLayout {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		} else if (child instanceof ViewGroup) {// 濡傛灉鏄鍚堟帶浠� // 閬嶅巻澶嶅悎鎺т欢
	...
	/** 鏌ユ壘澶嶅悎鎺т欢骞惰缃畆adiobutton */
	private void setCheckedView(ViewGroup vg) {
		int len = vg.getChildCount();
		for (int i = 0; i < len; i++) {
			if (vg.getChildAt(i) instanceof RadioButton) {// 濡傛灉鎵惧埌浜嗭紝灏辫缃甤heck鐘舵�
			...
			} else if (vg.getChildAt(i) instanceof ViewGroup) {// 杩唬鏌ユ壘骞惰缃�
		...
		public void onChildViewAdded(View parent, View child) {
			if (parent == FlowRadioGroup.this && child instanceof RadioButton) {
				...
				((RadioButton) child)
					...
					&& child instanceof ViewGroup) {// 濡傛灉鏄鍚堟帶浠� // 鏌ユ壘骞惰缃甶d
